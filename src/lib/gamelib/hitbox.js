...
HitBox.prototype.setSegment = function (center, radius, angle1, angle2) {
  ...
  var endPoint1 = new Vector2(
    this._center.x + Math.cos(this._angle1) * this._radius,
    this._center.y + Math.sin(this._angle1) * this._radius
  );
  var endPoint2 = new Vector2(
    this._center.x + Math.cos(this._angle2) * this._radius,
    this._center.y + Math.sin(this._angle2) * this._radius
  );
  this._lineSegmentCenter = new Vector2(
    (endPoint1.x + endPoint2.x) * 0.5,
    (endPoint1.y + endPoint2.y) * 0.5
  );

  // Solve bounding rectangle (not axis aligned)
  var towardsSegCenter = new Vector2(
    this._lineSegmentCenter.x - this._center.x,
  ...
  var enclosingPoint1 = new Vector2(endPoint1.x, endPoint1.y);
  enclosingPoint1.addScaledVector(towardsSegCenter, 1);
  var enclosingPoint2 = new Vector2(endPoint2.x, endPoint2.y);
  enclosingPoint2.addScaledVector(towardsSegCenter, 1);
  this._boundingPolygon = new Polygon([
    endPoint1,
    endPoint2,
    enclosingPoint2,
    enclosingPoint1,
...
HitBox.prototype.setPolygon = function (polygon) {
  this._shape = HitBox.Shape.POLYGON;
  this._boundingPolygon = polygon;
};
...
HitBox.prototype.intersects = function (other) {
  ...
  if (that._shape === HitBox.Shape.VEC2) {
    if (other._shape === HitBox.Shape.VEC2) {
      return (
        that._shape.x === other._shape.x && that._shape.y === other._shape.y
      );
      ...
      // segment edge.
      var segmentCenterAngle = mathUtil.mixAngles(
        other._angle1,
        other._angle2,
        0.5
      );
      var pointAngle = Math.atan2(
        that._pos.y - other._center.y,
        that._pos.x - other._center.x
      );
      pointAngle = mathUtil.angleDifference(pointAngle, segmentCenterAngle);
      var distAlongSegmentNormal = Math.cos(pointAngle) * dist;
      return (
        distAlongSegmentNormal >
        other._lineSegmentCenter.distanceTo(other._center)
      );
    ...
    if (other._shape === HitBox.Shape.CIRCLE) {
      return (
        other._center.distanceTo(that._center) < other._radius + that._radius
      );
    } else if (other._shape === HitBox.Shape.SEGMENT) {
      if (
        other._center.distanceTo(that._center) >=
        other._radius + that._radius
      ) {
        return false;
      }
      return other._boundingPolygon.intersectsCircle(
        that._center,
        that._radius
      );
    } else if (other._shape === HitBox.Shape.POLYGON) {
      return other._boundingPolygon.intersectsCircle(
        that._center,
        that._radius
      );
    ...
    if (other._shape === HitBox.Shape.SEGMENT) {
      ...
      if (
        !other._boundingPolygon.intersectsCircle(that._center, that._radius)
      ) {
        return false;
      }
      return that._boundingPolygon.intersectsCircle(
        other._center,
        other._radius
      );
    }
  } else if (that._shape === HitBox.Shape.POLYGON) {
...
HitBox.prototype.render = function (ctx, pointScale) {
  ...
  if (this._shape === HitBox.Shape.VEC2) {
    var pos = this._pos;
    ctx.fillRect(
      pos.x - pointScale * 0.5,
      pos.y - pointScale * 0.5,
      pointScale,
      pointScale
    );
  } else if (this._shape === HitBox.Shape.RECT) {
  ...
  } else if (this._shape === HitBox.Shape.POLYGON) {
    this._boundingPolygon.renderStroke(ctx, 'green');
  }
};
