...
var bezierLength = function (x0, y0, x1, y1, x2, y2, steps) {
  ...
  for (var i = 0; i < steps; ++i) {
    t += 1.0 / steps;
    xd =
      x0 * Math.pow(1.0 - t, 2) + x1 * t * (1.0 - t) * 2 + x2 * Math.pow(t, 2);
    yd =
      y0 * Math.pow(1.0 - t, 2) + y1 * t * (1.0 - t) * 2 + y2 * Math.pow(t, 2);
...
export { angleDifference, angleGreater, bezierLength, binomialCoefficient, clamp, ease, factorial, fmod, mix, mixAngles, mixSmooth, mixWithWrap, };
