...
Polygon.prototype.getSegments = function (vec) {
  const segments = [];
  for (var i = 0; i < this._vertices.length; ++i) {
    let next = i + 1;
    if (next >= this._vertices.length) next = 0;
    const segment = {
      a: this._vertices[i],
      b: this._vertices[next],
    };
...
Polygon.prototype.intersectsCircle = function (center, radius) {
  ...
  for (var i = 0; i < this._vertices.length; ++i) {
    ...
    var d= projected.distanceTo(center)
    if (d <= radius) {
      return { p1: vert1, p2: vert2, v: projected }
    }
  }
...
Polygon.prototype.intersectsPolygon = function (other) {
  for (var i = 0; i < this._vertices.length; ++i) {
    ...
    for (var j = 0; j < other._vertices.length; ++j) {
      if (
        Polygon.lineSegmentsIntersect(
          this._vertices[i],
          this._vertices[(i + 1) % this._vertices.length],
          other._vertices[j],
          other._vertices[(j + 1) % other._vertices.length]
        )
      ) {
 ...
 */
Polygon.prototype.renderStroke = function (ctx, color) {
  ctx.beginPath();
